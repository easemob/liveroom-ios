...
@interface LRGCDMulticastDelegateNode : NSObject {
@private
...
@interface LRGCDMulticastDelegate ()
{
...
@interface LRGCDMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation LRGCDMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	@synchronized(delegateNodes){
        LRGCDMulticastDelegateNode *node =
	    [[LRGCDMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    @synchronized(delegateNodes){
        ...
        {
            LRGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i-1)];
...
{
    @synchronized(delegateNodes){
        for (LRGCDMulticastDelegateNode *node in delegateNodes)
        {
            node.delegate = nil;
            #if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
            node.unsafeDelegate = nil;
...
- (LRGCDMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[LRGCDMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
- (NSInvocation *)duplicateInvocation:(NSInvocation *)origInvocation
{
	...
	for (i = 2; i < count; i++)
	{
        ...
        else if (*type == *@encode(SecTrustRef))
        {
            void *value;
            [origInvocation getArgument:&value atIndex:i];
            [dupInvocation setArgument:&value atIndex:i];
...
@implementation LRGCDMulticastDelegateNode
...
@implementation LRGCDMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		LRGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
		currentNodeIndex++;
